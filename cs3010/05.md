...
$$
\boxed{
	\text{Base: } D(i,0) = \frac{ f(x_0 + k) - f(x_0 - k)}{2k} \\
} \\~\\
\boxed{
	\text{Extrapolate: } D(i,j) = D(i,j-1) + \frac{D(i, j-1) - D(i-1, j-1)}{4^j - 1}
} \\
\small\textit{initial step size: $h$; step size for row $i$: $k = h/2^i$}
$$
