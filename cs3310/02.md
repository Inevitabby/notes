...
> - Average Case: $C_\text{avg}(n) = (p(n+1))/2 + n(1-p)$
> 
> # How We Calculate Average Case:
> 
...
> ## Cases
> 
> 1. In the case of a successful search, the probability of the first match occurring in the $i$th position of the list is $p/n$ for every $i$, 
> 	- so, probability of successful search: $\Sigma p/n \times i$
> 	- *(and, the number of comparisons made by the algorithm in such a situation is $i$ (obviously))*
> 
> 1. In the case of an unsuccessful search, the number of comparisons will be $n$ with the probability of such a search being $(1-p)$
> 	- so, probability of unsuccessful search: $n(1-p)$
> 	- *aka, # of comparisons ($n$) multiplied by the chance of an unsuccessful search ($1-p$).*
> 
> The total probability we want is simply the addition of these two probabilities.
> 
> ## Simplification
> 
> $$
