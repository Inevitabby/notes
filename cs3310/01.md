...
**Algorithm**: Sequence of unambiguous instructions[^unambiguous] for solving a problem in a finite amount of time[^finite_time].

[^unambiguous]: Meaning each step is clearly defined and leaves no room for subjective interpretation.
[^finite_time]: An algorithm must eventually terminate. If it runs forever, it is a process, not an algorithm.
...
2. Decide on computational means[^comp_means]
3. Design an algorithm
4. Prove correctness
5. Analyze the algorithm
6. Code the algorithm

[^comp_means]: e.g., will the solution be exact or approximate?
...
> - Check $t=24$: $60 \bmod 24 \neq 0$.
> - Check $t=23$: $60 \bmod 23 \neq 0$.
> - ...
> - Check $t=12$: $60 \bmod 12 = 0$ and $24 \bmod 12 = 0$. We return 12.
...
Apply the above equality repeatedly until the second parameter becomes zero. At this point the problem becomes trivial to solve[^gcd_zero].

[^gcd_zero]: Because $\gcd(x,0) = x$.
...
Key Property: It's easier to tell if a number *isn't* prime[^composite_property]
...
[^composite_property]: A composite number must have a prime factor less than or equal to its square root.
